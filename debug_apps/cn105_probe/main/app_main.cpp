...
async function saveStep2(){if(await writeSettings('nvs_out','正在保存第二步完整配置到 NVS...')){setStepEnabled(3,true);set('ota_out','第二步已保存。现在可以上传 firmware_exports/<version>/ 里的正式固件 app bin（文件名带 _0x20000.bin）。');$('step3').scrollIntoView({behavior:'smooth',block:'start'})}}
