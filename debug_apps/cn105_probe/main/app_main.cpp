...
<section class="card locked" id="step3"><h2>第三步：上传正式固件 OTA</h2><div class="step-note">第二步保存后才能上传正式固件。请选择 <code>firmware_exports/&lt;version&gt;/</code> 里的 app bin，也就是带 <code>_0x20000.bin</code> 的那个文件。</div><input id="fw" type="file" accept=".bin"><div class="btns"><button class="primary" onclick="upload()">上传正式固件</button><button onclick="applyOta()">重启应用 OTA</button></div><pre id="ota_out">请先完成第二步保存。</pre></section>
</main><script>
...
async function saveStep2(){const ssid=$('wifi_ssid').value.trim();const pass=$('wifi_pass').value.trim();if(placeholderWifi(ssid,'YOUR_WIFI_SSID')||placeholderWifi(pass,'YOUR_WIFI_PASSWORD')){set('nvs_out','请先填写真实的 WiFi SSID 和密码，不能保留默认占位值。');$('wifi_ssid').focus();return}if(await writeSettings('nvs_out','正在保存第二步完整配置到 NVS...')){setStepEnabled(3,true);set('ota_out','第二步已保存。现在可以上传 firmware_exports/<version>/ 里的正式固件 app bin（文件名带 _0x20000.bin）。');$('step3').scrollIntoView({behavior:'smooth',block:'start'})}}
