# Codex 傻瓜式迁移指南：三菱 CN105 ESPHome → ESP HomeKit SDK
...
- 当前生产方向是 `ESP-IDF + Espressif esp-homekit-sdk`
- 不再使用 `HomeSpan` 作为生产 HomeKit 路径
- 不再使用 `esp-matter` 作为当前迁移目标
- 后文大量 Arduino/HomeSpan 内容只作为历史参考；实际执行以本地规则、`PLAN.md`、当前 ESP-IDF 代码为准
...
- 再逐步接入 CN105 协议层
- 最后再接 `esp-homekit-sdk` / HomeKit 集成
...
- 当前 ESP-IDF 迁移版只使用 STA 模式
- Wi-Fi 连接失败时不要回退开启 AP
- 无 Wi-Fi credentials 时保持离线状态并通过日志提示，不启动 fallback AP
- 后续如果要使用 provisioning，应优先参考 `esp-homekit-sdk/examples/common`，不要临时恢复旧 AP fallback 行为
